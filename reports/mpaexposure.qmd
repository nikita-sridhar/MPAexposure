...
make_anom_map <- function(mpa_summary_df, col){
  
  anom_points <- st_as_sf(mpa_summary_df, coords = c("degx","degy")) 
                          
  tm_shape(CA_shp) + 
    tm_polygons() +
    
  tm_shape(anom_points) +
    tm_dots(col = num_event)

}

make_anom_map(temp_mpa_summary, periodt)

#non function scratch
anom_points <- st_as_sf((temp_mpa_summary %>% filter(period == "endcen")), 
  ...
  tm_shape(anom_points) +
    tm_dots(col = "num_event", size = 0.05) +
    
    
  tm_layout(bg.color = "#a6cee3", 
            title = "Number of Anomalous DO Events")
