...
pH_event_summary <- ph_do_events %>%
  ...
  mutate(duration_days = if_else(is_pH_low == "TRUE", n(), NA),
         duration_days = if_else(duration_days == 1, NA, duration_days), #want 1 day event to register as no event
         pH_event_id = if_else(duration_days == 0, NA, pH_event_id),#want to just make non events an NA
...
distinct(File, NAME, degx, degy, period, avg_annual_days_below_pH_thresh, avg_annual_ph_belowthresh) 
