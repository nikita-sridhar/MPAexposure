...
fviz_nbclust(scores[,1:2], FUNcluster = kmeans, method = "wss", k.max = 8) 
fviz_nbclust(scores[,1:2], FUNcluster = kmeans, method = "silhouette", k.max = 8) 
fviz_nbclust(scores[,1:2], FUNcluster = kmeans, method = "gap_stat", k.max = 8) 
#both gap_stat and sillhouette say 2 clusters

#cluster: first set seed and run k means
set.seed(123)
km <- kmeans(scores[,1:2], centers = 2, nstart = 50)
...
#still need this even if no cluster (for lines across mpa)
...
mpa_change <- factors %>%
  ...
  mutate("PC1_Historic_Mid-cen_change" = PC1_midcen - PC1_historic,
         "PC1_Mid-cen_End-cen_change" = PC1_endcen - PC1_midcen,
         "PC2_Historic_Mid-cen_change" = PC2_midcen - PC2_historic,
  ...
  select(-PC1_endcen,-PC1_historic,-PC1_midcen,-PC2_endcen,-PC2_historic,-PC2_midcen) %>%
...
arrange(PC, region) %>%
  ...
  #opacity for graph
  mutate(opacity = ifelse(Period == "Historic → Mid-cen", 0.4, 1),
...
ggplot(region_mpa_change_summary, aes(x = region, y = mean_pc_change, 
  ...
  scale_alpha_identity(name = "Period",
                       labels = c("Historic → Mid-cen","Mid-cen → End-cen"),
                       breaks = c(0.4,1),
                       guide = "legend") +
...
pc1_hist_midcen_mod <- lm(PC1_midcen ~ PC1_historic, data = factors)
pc1_hist_endcen_mod <- lm(PC1_endcen ~ PC1_historic, data = factors)

pc2_hist_midcen_mod <- lm(PC2_midcen ~ PC2_historic, data = factors)
pc2_hist_endcen_mod <- lm(PC2_endcen ~ PC2_historic, data = factors)

#saving residuals
pc_residuals <- factors %>% 
  
  mutate(pc1_hist_midcen = residuals(pc1_hist_midcen_mod),
         pc1_hist_endcen = residuals(pc1_hist_endcen_mod),
         pc2_hist_midcen = residuals(pc2_hist_midcen_mod),
         pc2_hist_endcen = residuals(pc2_hist_endcen_mod)) %>%
  ...
  #opacity for graph
  mutate(opacity = ifelse(Period == "hist → midcen", 0.4, 1),
         region = factor(region, levels = c("norca", "centralca", "channel", "socal")),
         Period = factor(Period, levels = c("hist → midcen", "hist → endcen")))
...
ggplot(pc_residuals, aes(x = region, y = region_mean, 
  ...
  scale_alpha_identity(name = "Period",
                       labels = c("hist → midcen" ,"hist → endcen"),
                       breaks = c(0.4,1),
                       guide = "legend") +
  facet_wrap(~PC, scales = "free")+
  theme_bw()
...
factors_pc2 <- as.data.frame(pca_all$x) %>%
  ...
  pivot_wider(id_cols = c("File","region","SHORTNAME"),
              ...
              values_from = "PC2",
              names_prefix = "PC2_") %>%
  select(File, region, SHORTNAME, PC2_historic, PC2_midcen, PC2_endcen)

#running lm model of historic vs. midcen and historic vs. endcen
hist_midcen_mod_pc2 <- lm(PC2_midcen ~ PC2_historic, data = factors_pc2)
hist_endcen_mod_pc2 <- lm(PC2_endcen ~ PC2_historic, data = factors_pc2)
midcen_endcen_mod_pc2 <- lm( PC2_endcen ~ PC2_midcen, data = factors_pc2)

summary(hist_midcen_mod_pc2)
summary(hist_endcen_mod_pc2)

#saving residuals and fitted values. - scaled so wrt 0 (not sure if need to scale or not)
factors2_pc2 <- factors_pc2 %>% 
  mutate(residual_hist_midcen = residuals(hist_midcen_mod_pc2),
         residual_hist_endcen = residuals(hist_endcen_mod_pc2),
         residual_midcen_endcen = residuals(midcen_endcen_mod_pc2),
         scaled_residual_hist_midcen = as.numeric(scale(residuals(hist_midcen_mod_pc2))),
         scaled_residual_hist_endcen =  as.numeric(scale(residuals(hist_endcen_mod_pc2))),
         scaled_residual_midcen_endcen =  as.numeric(scale(residuals(midcen_endcen_mod_pc2))),
         scaled_PC2_historic = scale(factors$PC2_historic)) 
...
bestfit <-lm(factors2_pc2$scaled_residual_hist_endcen ~ factors2_pc2$scaled_residual_hist_midcen)
...
factors3_pc2 <- factors2_pc2 %>%
  mutate(dist_to_line = abs(slope * scaled_residual_hist_midcen - 
                              scaled_residual_hist_endcen + intercept)
                        / sqrt(slope^2 + (-1)^2)) %>%
  mutate(label = case_when(dist_to_line > 1.3 ~ SHORTNAME, 
                           abs(scaled_residual_hist_midcen) > 1.5 & 
                           abs(scaled_residual_hist_endcen) > 1.5 ~ 
                          SHORTNAME, .default = NA))


#historic vs midcen residual plot - added scaled predicted to scaled residual
pc2 <- ggplot(factors3, aes(x = scaled_residual_hist_midcen, y = scaled_residual_hist_endcen)) +
